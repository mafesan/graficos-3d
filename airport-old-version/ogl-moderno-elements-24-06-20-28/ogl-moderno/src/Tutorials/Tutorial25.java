 ...
 * @author Agustin
 */
public class Tutorial25 implements Drawable {

    private ShaderProgram shaderProgram;
    private final OpenGLHelper openGLHelper = new OpenGLHelper("Tutorial 25", new FPCameraController(0, 0, 0));
    private int uniModel;
    private int uNMatrixAttribute;

    private final SphereModel sphereModel = new SphereModel(30);

    public void run() {
        openGLHelper.initGL("VS_ADS_Light.vs", "FS_GoraudLight.fs");
        prepareBuffers();
        initLights();
    ...
    private void initLights() {
        ...
        int uLaAttribute = shaderProgram.getUniformLocation("La");
        int uLdAttribute = shaderProgram.getUniformLocation("Ld");
        int uLsAttribute = shaderProgram.getUniformLocation("Ls");
        ...
        glUniform3f(uLaAttribute, 0.4f, 0.4f, 0.4f);
        glUniform3f(uLdAttribute, 0.5f, 0.5f, 0.5f);
        glUniform3f(uLsAttribute, 0.9f, 0.9f, 0.9f);
