...
public class Tutorial12 {
    ...
    public void run() {
        System.out.println("Hello LWJGL " + Sys.getVersion() + "!. Tutorial 12");
...
static final String VertexShaderSrc = 
    ...
    private void initGL() {
        ...
        // Create the window
        window = glfwCreateWindow(WIDTH, HEIGHT, "Tutorial 12", NULL, NULL);
        if (window == NULL) {
    ...
    private void drawTriangle1() {
            /* Render triangle */
            float posX = (float)Math.cos(count);
            float posY = (float)Math.sin(count);
    ...
    private void drawTriangle2() {
            ...
            Matrix4f model = Matrix4f.translate(posX, posY, 0);
            model = model.multiply(Matrix4f.rotate(count*100, 0, 0, 1));
            glUniformMatrix4(uniModel, false, model.getBuffer());
            glDrawArrays(GL_TRIANGLES, 0, 3);
    }
}
