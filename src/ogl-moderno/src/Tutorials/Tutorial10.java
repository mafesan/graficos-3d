...
public class Tutorial10 {
    ...
    public void run() {
        System.out.println("Hello LWJGL " + Sys.getVersion() + "!. Tutorial 10");
...
static final String VertexShaderSrc = 
    ...
    private void initGL() {
        ...
        // Create the window
        window = glfwCreateWindow(WIDTH, HEIGHT, "Tutorial 10", NULL, NULL);
        if (window == NULL) {
    ...
    private void loop() {
        ...
        while (glfwWindowShouldClose(window) == GL_FALSE) {
            ...
            /* Render triangle */
            count += 0.01f;
            float escaleX = (float)Math.sin(count);
            float escaleY = (float)Math.cos(count);
            Matrix4f model = Matrix4f.scale(escaleX, escaleY, 1);
